...
* XCの仕様では引数が`int n`のため負数が渡される可能性があり、その場合問題が生じるが、そもそもXCの実装(JSTRNCMP.S)では
  ...
  * `jstrncmp()`自体のプロトタイプ宣言はない(`#define`で別名定義している)ので、`jstrncmp()`の引数を`size_t n`にする
    かどうかを考える必要がない。
...
* プロトタイプ宣言の引数に変数名を追加する(宣言を見ただけで引数の内容が分かるようにするため)。
* プロトタイプ宣言の引数の変数名、インライン関数の引数名や変数名の先頭に`_`をつける(ユーザー定義マクロとの衝突回避)。
  * [glibc](https://github.com/bminor/glibc/blob/master/include/stdio.h)では`__`になっている。
* X680x0独自の2バイト半角文字コードに対応したマルチバイト文字関数群。
* 起動時の対応MPU検査を`btst d0,#__MPUTYPES`のようにして、任意の指定ができるようにする
  (例:68030と68040はOKで、68000～68020と68060はNGのような指定)。
...
* gcc2のときinline版`_dos_super()`を定義するようにしていたが、すべての環境で有効にする。
  また、defer-pop対策を自前で行うようにする。
...
* 改造版の名称、ライセンスを再検討してリブート(20026年を目標)
  * ライセンスは0BSDにする予定。GPLv3のランタイムライブラリ例外つきだと小回りが効かない
    (改変版をソースコードなしで配布されるのは望ましくないが、それをやる人はいないだろうから)。
...
* p.253 `realloc()` 「sizeがその余裕で吸収することができたり，現在のサイズよりも小さい場合はメモリ領域は移動せず」とあり
  LIBCの実装ではそのようになっているが、規格上は移動があり得るし、macOSなど移動する実装も実在する。
