...
## `jstrncmp()` (JSTRNCMP.S)
* 引数`int n`が0のとき、文字列が一致しなくなるかNUL文字に到達するまですべて比較してしまう。
* 引数`int n`が負数のとき、XCの実装では`n`を符号なしとして扱っているため、
  文字列が一致しなくなるかNUL文字に到達するまですべて比較してしまう。
...
## CライブラリマニュアルVOL2
* P.362 SPALET
  * 引数`int CODE`の内容は、ビット31が垂直帰線期間の検出、最下位バイトがパレットコードの指定。
* P.370 SP_REGST
  * (参考情報) ver2.0では`int MODE`という引数があるが間違い。垂直帰線期間検出は`SPRITENO`の最上位ビットで設定する。
    ver2.1 NEW KITでは訂正されている。
...
* P.255 M_OUT
  * 入力`D2.B`の「出力データ(0～225)」の上限値は255が正しい。
* P.259
  * 入力の`D2.W`と`D3.L`が逆。正しくは`D2.L`がバッファサイズ、`D3.W`がサンプリング周波数。
* P.265 M_MDREG
  * 入力に`D3.W レジスタへ書き込むデータ(0～255)`がない(`D3.W`のサイズはM_OPMREGに倣った)。
  * 備考：間違ったアドレスに書き込みを行ってしまうので使わないように。
* P.272 M_PGMFLT
  * 入力の`D2.W`は`D2.L`が正しい。
* P.273 M_VOLFLT
  * 入力の`D2.W`は`D2.L`が正しい。
* P.274 M_PANFLT
  * 入力の`D2.W`は`D2.L`が正しい。
