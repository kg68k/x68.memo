...
# TwentyOne +Cで問題が発生するもの

## DIR ARRANGER ver2.03 (ARRANGER.R)
* ディレクトリ名を大文字に変換しているため、小文字ディレクトリ名だと「ディレクトリがありません。」というエラーになる。

## MDX file dis converter v0.18.8 (dm.x)
* 拡張子を`.mdx`に変換しているため、大文字拡張子だと「ファイルが見つかりません。」というエラーになる。
* ちなみに拡張子を小文字に変換しているのではなく`.mdx`に差し替える方式のため、`.mdx`以外の拡張子はすべてエラーになる。
