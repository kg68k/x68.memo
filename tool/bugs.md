...
* [memcpy.S](https://github.com/Lydux/newlib-1.19.0-human68k/blob/master/newlib/libc/machine/m68k/memcpy.S)
  ...
  * [elf2x68kでは修正済み #34](https://github.com/yunkya2/elf2x68k/issues/34)。2026-03-22現在は未リリース。
...
### pilib.x v0.18 (ras68k-ext)
* 備考: 以前アップロードされていたPILIB018.LZHに入っているpilib.xはv0.16。2026-05-31以降はv0.18に差し替えられている。
* 備考: 常駐すると`trap #5`と同じ割り込みハンドラが`trap #6`にも設定される。
* 常駐部の探索時に、常駐プロセスではないメモリブロックを誤認する。起動したpilib.xより大きいアドレスにある常駐部を認識しない。
...
### 9vae.x 003
* 起動時に`…\9va\9va_data\Localizable.strings`というHuman68kで扱えないファイル名を開こうとして失敗している。
  ([@kamadox/2041432806585479350](https://x.com/kamadox/status/2041432806585479350))
* 起動時にバスエラーが発生する。
  * `A:/ed2/9va/9va_data/afa9va.png`が存在する状態で起動すればよい。
    ([@kg68k/2041511336237232622](https://x.com/kg68k/status/2041511336237232622))
...
### AJOY.X version 1.00
* デバイスドライバとしての機能は持っていないが無効デバイス名にしていないため(デバイス名`JOY`)、デバイスドライバへアクセスできてしまう。
  * 入力するとゴミデータが入力される。
